...
M13 exposes the first memory API under `/v1/memory`. All responses use safe summaries and omit raw memory content from list/search surfaces. M14 prepares the management/audit contract and unifies list/search filters so the Settings > Memory UX can be implemented without fake history.
