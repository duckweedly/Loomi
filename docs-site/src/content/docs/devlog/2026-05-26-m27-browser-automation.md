...
- Added builtin `browser.open`, `browser.snapshot`, and `browser.click_link` catalog identity, default persona allowlist, Work-mode filtering, and safe tool-call metadata grouping.
- Added `BrowserToolExecutor` with run-scoped sessions, HTTP(S)-only URL validation, credential rejection, private/local network denial, redirect validation, timeout/byte bounds, title/text/link extraction, and bounded safe result summaries.
...
No Chrome profile control, cookies, JavaScript rendering, form fill, screenshots, downloads, authenticated browsing, crawler, search provider, artifact runtime, activity recorder, plugin marketplace, channels, heartbeat, or multi-agent orchestration were added.
