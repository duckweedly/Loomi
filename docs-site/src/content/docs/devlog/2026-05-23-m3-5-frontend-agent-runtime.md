...
M3.5 增加了前端 Agent runtime 骨架，让 Loomi 在真实 M4/M5 后端完成前也能验证一次 Chat execution loop。
...
- Real adapter 在 M4/M5 之前报告 unavailable，不做隐藏 mock fallback。
...
# passed, 23 pages built
```

浏览器 smoke：

- Mock success：提交 Chat 消息后，用户消息立即出现；Timeline 显示 6 个里程碑；Agent 状态进入 Done；Chat Canvas 显示“已完成”。
- Mock failure：在 Run Rail 选择“失败剧本”后提交消息，Timeline 只显示一个 `run.failed` terminal event；Chat Canvas 显示“执行失败”；没有追加成功 assistant 回复。
...
- Failure script 的选择入口还不是正式产品控件；测试可通过 mock script selector/helper 验证。
- Real API mode 只暴露 runtime unavailable，不实现 run/event/SSE。
- Work mode 暂不接 runtime execution，只保留 mode-specific thread 列表和现有行为。

## 下一步

- M4：实现持久化 run/event 和 SSE stream。
