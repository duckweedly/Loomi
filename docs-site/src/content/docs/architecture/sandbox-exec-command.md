...
M78 borrows only the mechanism shape: process ids, run/session ownership, bounded output snapshots, explicit continue/terminate actions, and terminal summaries. M81 tightens the next small gap by making stdout cursor semantics stable across long output: `next_cursor` is an absolute captured-byte cursor, the retained preview is bounded to the latest safe tail, and terminal `continue_process` calls return stored state without attempting new stdin writes. M93 adds the service-before-service layer for restore: process handles are snapshotted to a repository, restored into a rebuilt registry, reconciled when the OS process is gone, expired through TTL cleanup, and exposed through safe UI summaries. The real API/worker path now wires that repository to `productdata.PostgresRepository`, so terminal and expired summaries survive API process restart when Postgres is available. Loomi does not copy Arkloop's config format, tier names, agent protocol, endpoint names, copy, or expression layer.

M93 deliberately does not implement Arkloop's full isolation model. There is no Firecracker microVM, Docker pool, guest user, network namespace, filesystem snapshot sync, artifact extraction, shell checkpoint, PTY resize, browser tier, warm pool, or sandbox template runtime. The current Loomi foundation is a local process registry around host `exec.Cmd`, backed by a minimal productdata/Postgres process record repository and guarded by Work-mode tool availability, explicit approval, argv-only validation, workspace cwd resolution, timeout/output bounds, and event redaction.
...
Process tools use a run-scoped process store with a small durable-record restore boundary:
...
- The productdata repository stores `run_id`, `process_id`, safe argv summary, cwd alias, status, cursor, byte counters, started/updated/ended timestamps, exit code, bounded stdout/stderr tail, stdin state, and terminal summary.
- Durable process-record writes are part of the tool result boundary. If initial start, continue/stdio state, terminate state, TTL/lost reconciliation, or completion state cannot be saved, Loomi surfaces a safe tool failure instead of returning a successful process result with an untrusted restore record.
- On registry rebuild or API process restart, terminal records remain readable but cannot accept stdin or close mutations. A restored `running` record without a live local command is marked `lost` with a safe terminal summary instead of starting a replacement process.
- TTL reconciliation marks over-lifetime or idle records `expired`. Record cleanup touches only Loomi-owned rows in `sandbox_process_records`; Loomi does not scan or kill unrelated host processes.
...
- API restart recovery is summary-level only: terminal, expired, and lost records can be read back from productdata/Postgres, but running OS processes are not reattached and commands are never restarted.
