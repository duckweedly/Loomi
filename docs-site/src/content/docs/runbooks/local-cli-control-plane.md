...
go run ./cmd/loomi runs attach <run-id> --host http://127.0.0.1:8080 --compact
...
7. `loomi doctor` reports config, API readiness, configured provider status, and tool catalog health; it exits non-zero when API readiness fails.
...
19. `loomi runs attach <run-id>` renders the run projection, replays stored events, then continues streaming from the last replayed sequence.
...
23. `loomi config show` reads `~/.loomi/config.json` or `LOOMI_CONFIG`, then applies `LOOMI_HOST`, `LOOMI_MODE`, `LOOMI_PROVIDER`, `LOOMI_MODEL`, `LOOMI_PERSONA`, and `LOOMI_SCRIPT` overrides.
24. `loomi config set/unset` writes only the config file, creates it with `0600`, and leaves env overrides untouched.
25. `loomi approvals follow` filters the event stream down to tool approval notices and copyable approve/deny commands.
26. `loomi tools list` text output groups tools by catalog group and supports `--group`, `--enabled-only`, and `--flat`.
27. `loomi events tail --tools-only --compact` filters out model delta/final events and renders compact tool call state, arguments, and per-tool result summaries.
28. `loomi mcp servers` renders safe MCP status rows without raw command, args, env, secrets, or host paths.
29. `loomi lsp tools` filters the tool catalog to LSP tools and keeps execution behind Work-mode approval-gated runs.
30. `loomi artifacts list/read` uses thread-scoped read-only artifact endpoints and renders bounded text excerpts only.
31. `loomi memory list/search/show/audit` uses safe memory APIs and does not write, approve, deny, or delete memory.
32. `loomi agent tasks/tools` exposes coordination-only task state and catalog entries without launching child model runs.
33. `loomi browser tools/events` exposes browser tool catalog entries and browser run events without direct browser session control.
