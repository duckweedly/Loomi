...
go test ./internal/runtime -run 'TestGatewayContinuation'
```
...
- non-workspace continuation still failing with `unsupported_tool_loop`
- safe todo metadata normalization and Work-mode-only replay projection
