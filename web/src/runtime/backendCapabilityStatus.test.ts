...
describe('backend capability status', () => {
  test('applies capability precedence from recovering through mock', () => {
    ...
    expect(deriveBackendCapabilityStatus({ dataSourceMode: 'real_api', runtimeSource: 'real_model' })).toBe('real-model')
    expect(deriveBackendCapabilityStatus({ dataSourceMode: 'real_api', backendUnavailable: true, runtimeSource: 'real_model' })).toBe('backend-unavailable')
  ...
  test('keeps mock and local simulated copy distinct from real model execution', () => {
    ...
    expect(getBackendCapabilityCopy('real-model').detail).toContain('real provider')
  })
