...
func TestValidateLSPToolCallArguments(t *testing.T) {
	...
	for _, name := range []string{ToolNameLSPReferences, ToolNameLSPDefinition, ToolNameLSPHover} {
		input := RecordToolCallRequestInput{ToolCallID: "tc_lsp_pos", ToolName: name, ArgumentsSummary: map[string]any{"path": "src/main.go", "line": 3, "column": 6}, ArgumentsHash: "hash_lsp", ApprovalStatus: ToolCallApprovalRequired, ExecutionStatus: ToolCallExecutionBlocked}
		if _, err := ValidateToolCallRequestInput(input); err != nil {
			t.Fatalf("valid %s err = %v", name, err)
		}
	}
	cases := []RecordToolCallRequestInput{
		...
		{ToolCallID: "tc_lsp", ToolName: ToolNameLSPDefinition, ArgumentsSummary: map[string]any{"path": "src/main.go", "line": 3}, ArgumentsHash: "hash_lsp", ApprovalStatus: ToolCallApprovalRequired, ExecutionStatus: ToolCallExecutionBlocked},
		{ToolCallID: "tc_lsp", ToolName: ToolNameLSPHover, ArgumentsSummary: map[string]any{"path": "src/main.go", "column": 6}, ArgumentsHash: "hash_lsp", ApprovalStatus: ToolCallApprovalRequired, ExecutionStatus: ToolCallExecutionBlocked},
	}
