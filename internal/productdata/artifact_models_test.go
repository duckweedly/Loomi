...
func TestValidateArtifactToolCallArguments(t *testing.T) {
	create := RecordToolCallRequestInput{ToolCallID: "tc_artifact", ToolName: ToolNameArtifactCreateText, ArgumentsSummary: map[string]any{"title": " Notes ", "content": "hello", "max_bytes": 1024}, ApprovalStatus: ToolCallApprovalRequired, ExecutionStatus: ToolCallExecutionBlocked}
	input, err := ValidateToolCallRequestInput(create)
	if err != nil {
		t.Fatal(err)
	}
	if input.ArgumentsSummary["title"] != "Notes" {
		t.Fatalf("title was not normalized: %+v", input.ArgumentsSummary)
	}
