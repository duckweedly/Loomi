...
4. Verify the run emits `tool_call_requested`, `tool_call_approved`, `tool_call_executing`, `tool_call_succeeded`, and continuation events without a manual approval step.
5. Repeat for `workspace.read` and `workspace.grep`.
6. Verify traversal, absolute outside path, sensitive files, and symlink escape fail without leaking content.
...
5. Confirm requested, auto-approved, executing, succeeded, and failed states are legible in Work/Chat timelines.
