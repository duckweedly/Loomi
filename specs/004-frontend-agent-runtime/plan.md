...
M3.5 turns Loomi's frontend from a mostly static shell into a mock-driven Agent runtime skeleton while the LLM gateway and later worker/tool runtime are still under construction. After merging with M4, the frontend keeps deterministic success/failure mock scripts for mock mode and preserves M4 real API run/event/SSE for configured real API mode. The plan adds a clear Chat Canvas state model, one shared execution adapter boundary, and visible linkage between Chat Canvas, Run Timeline, and the Agent state motion badge.
...
**Testing**: `bun test ./web/src/**/*.test.ts "web/vite.config.test.ts"` for unit/contract-style tests; `bun run --cwd web build`; browser smoke for mock success/failure flow and M4 real API run/event/SSE mode; docs validation with `bun run --cwd docs-site build` when docs are updated.
...
**Constraints**: No LLM gateway; no worker queue; no tool execution; no new frontend framework; no hidden fallback from configured real API mode to mock run behavior; real API mode must use M4 run/event/SSE when configured; product UI microcopy stays sparse; learning documentation is Chinese and explanatory.
...
- **III. Core Flow Before Platform Complexity**: PASS. This is explicitly a frontend M3.5 bridge layered with M4 run/event/SSE; it does not pull forward LLM gateway, tools, workers, desktop runtime, attachments, RAG, or plugins.
...
- **V. Safety, Permissions, and Data Boundaries**: PASS. Real API mode must use M4 run/event/SSE when configured and must not silently fallback to mock execution; unavailable states remain reserved for genuinely missing future capabilities.
...
- Treat configured real API mode as M4 run/event/SSE capable after the M4 merge, and never use mock execution as a hidden fallback.
...
- **Runnable Vertical Slice**: PASS. Quickstart demonstrates success, failure, stopped, no-thread, empty-thread, loading/error, and M4 real API run/event/SSE conditions.
- **Core Flow Before Platform Complexity**: PASS. Contracts are frontend-first and explicitly defer LLM, tools, workers, desktop runtime, and platform features while preserving M4 run/event/SSE.
